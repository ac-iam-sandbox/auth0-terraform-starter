...
# This avoids hardcoding https://{domain}/api/v2/ per environment.
...
module "tenant" {
  ...
  env_config = lookup(local.env_config, "tenant", {})
}
...
module "attack_protection" {
  ...
  env_config = lookup(local.env_config, "attack_protection", {})
}
...
module "email_provider" {
  ...
  env_config = lookup(local.env_config, "email_provider", {})
}
...
module "guardian" {
  ...
  env_config = lookup(local.env_config, "guardian", {})
}

# 5. M2M Clients (multi-instance, env-filtered)
module "clients" {
...
# 7. Log streams (multi-instance, env-filtered)
module "log_streams" {
  source      = "./modules/log_streams"
  definitions = local.log_streams
  secrets     = local.secrets
  env_config  = lookup(local.env_config, "log_streams", {})
  environment = var.environment
}
